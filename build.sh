...
build_kernel() {
	...
	if [[ ${BUILD_PREF_COMPILER_VERSION} == 'rui' ]]; then
		...
		make -C $(pwd) CC=${BUILD_PREF_COMPILER} LD=ld.lld LLVM=1 LLVM_IAS=1 AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip -j$(nproc --all) LOCALVERSION="${LOCALVERSION}" 2>&1 | sed 's/^/     /'
	elif [[ ${BUILD_PREF_COMPILER} == 'clang' ]]; then
		make -C $(pwd) CC=${BUILD_PREF_COMPILER} LD=ld.lld LLVM=1 LLVM_IAS=1 ${BUILD_DEVICE_TMP_CONFIG} LOCALVERSION="${LOCALVERSION}" 2>&1 | sed 's/^/     /'
		make -C $(pwd) CC=${BUILD_PREF_COMPILER} LD=ld.lld LLVM=1 LLVM_IAS=1 -j$(nproc --all) LOCALVERSION="${LOCALVERSION}" 2>&1 | sed 's/^/     /'
	else
